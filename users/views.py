    ...
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            if request.user.profile.user_type == 'employer':
                ...
                return redirect(request.GET['next'] if 'next' in request.GET else 'accountStudent')
        else:
...
@login_required(login_url='login')
def userAccountStudent(request):
    profile = request.user.profile
    ...
    context = {'profile': profile, 'skills': skills, 'projects': projects}
    return render(request, 'users/account_student.html', context)
